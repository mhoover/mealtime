...
X_tr, X_cv, X_ts, y_tr, y_cv, y_ts = make_data(d, ['rest_name', 
	'delivery_hood'], ['online', 'open_orders', 'rain', 'drivers_working', 
	'hour'], tf = False)
...
for val in [m, m_ridge, rf, gb, by]:
	print('mse: ' + str(round(mean_squared_error(y_ts, val.predict(X_ts)), 2)))

for val in [mb, mb_ridge, rfb, gbb, byb]:
	print('mse: ' + str(round(mean_squared_error(trans(yb_ts, l), 
		trans(val.predict(X_ts), l)), 2)))

# no transform: gb, by, m, m_ridge, rf
# transform: m_ridge, m, by, gb, rf
...
for val in [m, m_ridge, rf, gb, by]:
	...
	print('within 10%: ' + str(round(np.mean(pct), 2)))

for val in [mb, mb_ridge, rfb, gbb, byb]:
	pct = trans(val.predict(X_ts), l) <= y_ts + y_ts * .1
	print('within 10%: ' + str(round(np.mean(pct), 2)))

# no transform: gb, m, by, m_ridge, rf
# transform: m_ridge, gb, m/by, rf
...
fname = np.array(['ArtCaffe', 'Osteria', 'Cedars', 'Sushi Soo', 'La Salumeria', 
	'Kilimani', 'Lavington', 'Westlands', 'Online', '# orders', 'Rain', 
	'# riders', 'Time of order'])
